...
- **واجب الغد (tomorrow's homework)** — an optional, multi-record surah/ayah list per student (add/remove rows, same UI pattern as حفظ جديد / مراجعة, just without rating/pages/mistakes fields), offline-only. It is never included in the Telegram payload and never persisted to the database; it exists solely to appear in the on-page report described below.
